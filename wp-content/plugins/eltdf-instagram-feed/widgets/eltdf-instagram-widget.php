...
class ElatedInstagramWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			'Elated Instagram Widget',
			array( 'description' => esc_html__( 'Display instagram images', 'readanddigest' ) )
		);
	...
	public function widget($args, $instance) {
		...
		if(is_array($images_array) && count($images_array)) {
			...
			if($title !== '') {
				?>
				<div class="eltdf-instagram-feed-heading">
			<?php
				echo $args['before_title'] . $title . $args['after_title'];
			?>
				<a href="<?php echo esc_url('https://www.instagram.com/'.$instagram_api->getScreenName());?>" target="_blank">
					<span class='fa fa-instagram'></span>
				</a>
			</div>
			<?php }
			?>
			<ul class="eltdf-instagram-feed clearfix eltdf-col-<?php echo $number_of_cols; ?>">
				<?php
				foreach ($images_array as $image) { ?>
					<li>
						<a href="<?php echo esc_url($instagram_api->getHelper()->getImageLink($image)); ?>" target="_blank">
							<?php echo readanddigest_kses_img($instagram_api->getHelper()->getImageHTML($image, $image_size)); ?>
							<span class="fa fa-instagram"></span>
						</a>
					</li>
				<?php } ?>
