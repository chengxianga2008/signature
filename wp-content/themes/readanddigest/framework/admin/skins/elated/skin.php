...
class ElatedSkin extends ReadAndDigestSkinAbstract {
    /**
     * Skin constructor. Hooks to readanddigest_admin_scripts_init and readanddigest_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('readanddigest_admin_scripts_init', array($this, 'registerStyles'));
        add_action('readanddigest_admin_scripts_init', array($this, 'registerScripts'));

        add_action('readanddigest_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('readanddigest_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('readanddigest_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('readanddigest_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            readanddigest_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            readanddigest_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = readanddigest_get_admin_tab();
        $active_page = $readanddigest_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param ReadAndDigestAdminPage $page current page to load
     *
     * @see ElatedSkinAbstract::loadTemplatePart()
     */
