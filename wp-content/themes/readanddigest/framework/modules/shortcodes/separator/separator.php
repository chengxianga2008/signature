<?php
namespace ReadAndDigest\Modules\Separator;
...
class Separator implements ShortcodeInterface{
	...
	public function vcMap() {
		...
		vc_map(
			array(
				'name' => esc_html__('Elated Separator', 'readanddigest'),
				'base' => $this->base,
				'category' => 'by ELATED',
	...
	public function render($atts, $content = null) {
		...
		$html = readanddigest_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
	...
	private function getSeparatorStyle($params) {
		...
		if ($params['width'] !== '') {
			if(readanddigest_string_ends_with($params['width'], '%') || readanddigest_string_ends_with($params['width'], 'px')) {
		...
		if ($params['top_margin'] !== '') {
			if(readanddigest_string_ends_with($params['top_margin'], '%') || readanddigest_string_ends_with($params['top_margin'], 'px')) {
		...
		if ($params['bottom_margin'] !== '') {
			if(readanddigest_string_ends_with($params['bottom_margin'], '%') || readanddigest_string_ends_with($params['bottom_margin'], 'px')) {
