<?php
namespace ReadAndDigest\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('readanddigest_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('readanddigest_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('readanddigest_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('readanddigest_after_header_connect');
    }
     ...
     *
     * Hooks to readanddigest_title_content_padding filter
     *
     * @return int
     */
