 ...
 */
if (!class_exists('ReadAndDigestStiCkynavigatIonwalker')) {
	class ReadAndDigestStiCkynavigatIonwalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($depth == 0){
				$out_div = '<div class="eltdf-menu-second"><div class="eltdf-menu-inner">';
			}else{
				$out_div = '';
			}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide") {
					if($item->icon != "" && $item->icon != 'null'){ 
						$menu_type = " eltdf-menu-wide eltdf-menu-wide-with-icons";
					} else { 
						$menu_type = " eltdf-menu-wide";
					}
				} elseif ($item->type_menu == "wide_custom_widget"){
					$menu_type = " eltdf-menu-wide eltdf-menu-custom-widget";
				} else {
					$menu_type = " eltdf-menu-narrow";
				}
			}
			...
			if($depth==0 && $item->type_menu != "wide_custom_widget"){
				if($item->wide_position == "right"){
					$wide_menu_position = " eltdf-menu-right-position";
				}elseif($item->wide_position == "left"){
					$wide_menu_position = " eltdf-menu-left-position";
				}else{
					$wide_menu_position = "";
				}
			}
			...
			// build html
			$output .= $indent . '<li id="sticky-menu-item-'. $item->ID . '" class="' . $class_names . ' '. $active . $sub . $menu_type . $wide_menu_position .'" '.$wide_background_image.'>';
			...
			if($item->hide == ""){
				...
				$angle_icon   = '';
				if($depth !== 0) {
					$angle_icon = '<i class="eltdf_menu_arrow arrow_carrot-right"></i>';
				}
				...
				if($item->icon_pack == 'font-awesome') {
					$icon .= ' fa';
				}
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<span class="eltdf_menu_arrow ion-chevron-down"></span>';
				}
